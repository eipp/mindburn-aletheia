...
class TokenEconomySimulation {
  ...
  private processDailyTokenomics() {
    // Process token burns
    const dailyBurn = this.tokenState.circulatingMBU.times(this.params.dailyBurnRate);
    ...
    const stakingRewards = this.tokenState.totalStaked.times(this.params.dailyStakingReward);
    this.tokenState.circulatingMBU = this.tokenState.circulatingMBU.plus(stakingRewards);

    // Process treasury operations
    const treasuryGrowth = this.tokenState.treasuryBalance.times(this.params.treasuryGrowthRate);
  ...
  private printFinalReport() {
    ...
    console.log('\nEconomic Metrics:');
    console.log(
      `- Treasury Growth: ${this.tokenState.treasuryBalance
        .div(new BigNumber(1e9))
        .times(100)
        .toFormat(2)}%`
    );
    console.log(
      `- Staking Ratio: ${this.tokenState.totalStaked
