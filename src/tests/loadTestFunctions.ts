...
export function generateDeviceFingerprint(context: any, events: any, done: () => void): void {
  const fingerprint = {
    ...
    plugins: Array.from({ length: faker.number.int({ min: 0, max: 5 }) }, () =>
...
export function generateWorkerProfile(context: any, events: any, done: () => void): void {
  const profile = {
    ...
    languages: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, () =>
      faker.helpers.arrayElement(['en', 'es', 'fr', 'de'])
    ),
    specializations: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, () =>
      faker.helpers.arrayElement(['text', 'image', 'audio'])
    ),
    availableHours: Array.from({ length: faker.number.int({ min: 1, max: 24 }) }, () =>
