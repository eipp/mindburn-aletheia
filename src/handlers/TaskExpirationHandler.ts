...
async function cleanupWorkerAssignments(task: any): Promise<void> {
  ...
  const pendingAssignments = await dynamodb
    .query({
      TableName: 'Results',
...
async function updateWorkerMetrics(task: any): Promise<void> {
  ...
  const assignments = await dynamodb
    .query({
      TableName: 'Results',
      KeyConditionExpression: 'taskId = :taskId',
      ExpressionAttributeValues: {
  ...
  const updates = assignments.Items.filter(assignment => assignment.status === 'ASSIGNED').map(
    assignment => updateWorkerResponseMetric(assignment.workerId)
  );
...
async function updateWorkerResponseMetric(workerId: string): Promise<void> {
  ...
  if (metrics.Item?.value < 0.5) {
    // 50% response rate threshold
    await dynamodb
      .update({
        TableName: 'Workers',
        Key: { workerId },
