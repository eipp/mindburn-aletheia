...
export async function cancelTask(taskId: string, reason: string): Promise<void> {
  ...
  if (
    ![TaskStatus.PENDING, TaskStatus.ASSIGNED, TaskStatus.IN_PROGRESS].includes(currentTask.status)
  ) {
  ...
  try {
    ...
    await dynamodb
      .update({
        ...
        Key: { taskId },
        UpdateExpression:
          'SET #status = :status, updatedAt = :updatedAt, cancellationReason = :reason, cancelledAt = :cancelledAt',
