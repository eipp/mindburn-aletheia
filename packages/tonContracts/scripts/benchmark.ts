...
async function runBenchmarks() {
    ...
    results.push(await measureGas(
        async () => await workerReward.sendCreateTask(client.getSender(), {
            taskId: 1,
            reward: toNano('10'),
            client: client
        }, toNano('10')),
    ...
    results.push(await measureGas(
        async () => await workerReward.sendAssignTask(owner.getSender(), {
            taskId: 1,
            worker: worker
        }),
        'Assign Task'
    ...
    results.push(await measureGas(
        async () => await workerReward.sendSubmitVerification(owner.getSender(), {
            taskId: 1,
            qualityScore: 90,
            taskComplexity: 2
        }),
    ...
    results.push(await measureGas(
        async () => await workerReputation.sendRegisterWorker(owner.getSender(), {
            worker: worker
        }),
        'Register Worker'
    ));
    ...
    results.push(await measureGas(
        async () => await workerReputation.sendUpdateReputation(owner.getSender(), {
            worker: worker,
            qualityScore: 90,
            taskComplexity: 2
        }),
    ...
    results.push(await measureGas(
        async () => await utilityToken.sendTransfer(owner.getSender(), {
            to: worker,
            amount: toNano('100')
        }),
        'Token Transfer'
    ...
    results.push(await measureGas(
        async () => await utilityToken.sendStake(worker.getSender(), {
            amount: toNano('1000')
        }),
        'Token Stake'
    ));
    ...
    results.push(await measureGas(
        async () => await workerReward.sendUpgrade(owner.getSender(), {
            newCode: newCode,
            delay: 3600
        }),
        'Initiate Upgrade'
    ...
    printResults(results.filter(r => 
        ['Create Task', 'Assign Task', 'Submit Verification'].includes(r.operation)
    ));

    console.log('\nWorkerReputation Contract:');
    printResults(results.filter(r => 
        ['Register Worker', 'Update Reputation'].includes(r.operation)
    ));

    console.log('\nUtilityToken Contract:');
    printResults(results.filter(r => 
        ['Token Transfer', 'Token Stake'].includes(r.operation)
    ));

    console.log('\nUpgrade Operations:');
    printResults(results.filter(r => 
        ['Initiate Upgrade', 'Complete Upgrade'].includes(r.operation)
    ));
}
