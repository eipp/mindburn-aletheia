...
export class QualityControlService {
  ...
  async calculateConsistencyScore(workerId: string, currentAccuracy: number): Promise<number> {
    try {
  ...
  async evaluateWorkerLevel(workerId: string): Promise<WorkerLevel> {
    try {
      ...
      } else if (qualityScore.overall >= this.thresholds.accuracy.low && history.totalTasks >= 20) {
  ...
  private async calculateQualityScore(
    ...
    const accuracyTrend = this.calculateTrend(history.recentTasks.map(task => task.accuracy));
    ...
    const overall =
      metrics.accuracy * 0.4 + consistency * 0.3 + speedScore * 0.2 + accuracyTrend * 0.1;
  ...
  private calculateSpeedScore(workerTime: number, averageTime: number): number {
