...
export const verificationScene = new Scenes.WizardScene<BotContext>(
  ...
  async ctx => {
    try {
      ...
      if (!taskResult.Item) {
        await ctx.reply('Task not found or no longer available.', {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [[{ text: '📋 View Other Tasks', callback_data: 'view_tasks' }]],
          },
        });
    ...
    try {
      ...
      if (ctx.callbackQuery.data === 'start_verification') {
        ...
        await ctx.reply(message, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [...keyboard, [{ text: '❌ Cancel', callback_data: 'cancel' }]],
          },
        });
  ...
  try {
    ...
    await ctx.editMessageText(
        ...
        (reward.txId ? `Transaction: ${ton.getExplorerUrl(reward.txId)}` : ''),
      {
        ...
        reply_markup: {
          inline_keyboard: [[{ text: '🔍 View Available Tasks', callback_data: 'view_tasks' }]],
        },
      }
    );
