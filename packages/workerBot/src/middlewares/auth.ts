...
export const authMiddleware = (dynamodb: DynamoDB.DocumentClient): Middleware<BotContext> => {
  return async (ctx: BotContext, next: () => Promise<void>) => {
    try {
      ...
      if (result.Item.status === 'banned') {
        ...
        await ctx.reply('Your account has been suspended. Please contact support for assistance.', {
          parse_mode: 'HTML',
        });
        return;
      }
      ...
      await ctx.reply('Authentication error. Please try again later.', { parse_mode: 'HTML' });
    }
  };
};
